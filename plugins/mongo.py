        ...
        if len(message.command) > 1:
            ...
            if len(message.command) == 3:
                ...
                if db_name in databases:
                    ...
                    await message.reply(
                        f"Collection `{col_name}` from database `{db_name}` has been deleted successfully. 🧹"
                    )
                else:
...
async def transfer_db_command(client, message: Message):
    try:
        if len(message.command) < 2:
            await message.reply(
                "Please provide the new MongoDB URL with the command: `/transferdb your_new_mongodb_url`"
            )
            return
        ...
        await message.reply(
            "Data backup from old MongoDB is complete. 📦\n\nNow opening new MongoDB and putting."
        )
