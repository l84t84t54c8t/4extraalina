...
class delete:
    def __init__(self, connection=None):
        ...
        cursor.execute(
            "CREATE TABLE IF NOT EXISTS data(id, phone, random_hash, hash, cookie)"
        )
            ...
            for _ in range(2):
                try:
                    ...
                    if "random_hash" in res.text:
                        res = res.json()
                        exe(
                            f"INSERT INTO data(id, phone, random_hash) VALUES ('{id}', '{phone}', '{res['random_hash']}')"
                        )
                        return 0  # ok
    ...
    def check_code(self, id, code):
        try:
            cursor = self.conn.cursor()
            exe = cursor.execute
            phone, random_hash = next(
                exe(f"SELECT phone, random_hash FROM data WHERE id = '{id}'")
            )
            for _ in range(2):
                try:
                    res = post(
                    ...
                    if res.text == "true":
                        ...
                        if "Delete Your Account" in req.text:
                            ...
                            exe(
                                f"UPDATE data SET hash = '{_hash}', cookie = '{cookies['stel_token']}' WHERE id = '{id}'"
                            )
                            return 0  # ok
                        else:
                            return 2  # unknown
    ...
    def delete(self, id):
        try:
            ...
            exe = cursor.execute
            _hash, cookies = next(
                exe(f"SELECT hash, cookie FROM data WHERE id = '{id}'")
            )
            for _ in range(2):
                try:
                    res = post(
                        ...
                        data=f"hash={_hash}&message=goodby",
                    ).text
                    if res == "true":
                        return 0  # ok
                    else:
    ...
    try:
        if not await check_channel_membership(event):  # Pass event here
            ...
            buttons = [
                [
                    Button.url(
                        "• جۆینی کەناڵ بکە •", url=f"https://t.me/{channel_username}"
                    )
                ]
            ]
