...
async def disable_protection(client, message):
    ...
    if (
        user_status.status in [ChatMemberStatus.OWNER, ChatMemberStatus.ADMINISTRATOR]
        ...
        return await message.reply_text(
            f"عذرًا {message.from_user.mention}, هذا الأمر لا يخصك✨♥"
        )
...
async def show_protection_settings(client, message):
    ...
    if message.chat.id in alkl:
        return await message.reply_text(
            f"عذرا {message.from_user.mention}, الأمر معطل من قبل مالك الجروب ✨✅"
        )
...
async def protection_options(client, CallbackQuery):
    ...
    if user_status.status not in [
        ...
        return await CallbackQuery.answer(
            "يجب أن تكون إدمن للقيام بذلك!", show_alert=True
        )
    ...
    await CallbackQuery.edit_message_text(
        "الآن تحكم في أوامر الحماية بالأسفل 👇",
        reply_markup=InlineKeyboardMarkup(buttons),
    )


# Further callback handlers for enabling and disabling protections can be
# simplified by using the above structure for each protection command.
        ...
        [
            [
                InlineKeyboardButton(
                    text="بالكتم",
                    callback_data=f"{action_prefix}_unmut{callback_suffix}",
                ),
                InlineKeyboardButton(
                    text="بالحظر",
                    callback_data=f"{action_prefix}_unban{callback_suffix}",
                ),
                InlineKeyboardButton(
                    text="بمسح الرساله",
                    callback_data=f"{action_prefix}_lock{callback_suffix}",
                ),
...
async def group_protection_handler(client: Client, CallbackQuery):
    ...
    # Get the protection type like 'photo', 'video', etc.
    action_prefix = command.split("_")[1]

    # Create buttons dynamically based on the protection type
    buttons = create_control_buttons(action_prefix, "1")

    await CallbackQuery.edit_message_text("اختار القيود الآن ✨♥", reply_markup=buttons)
...
async def open_protection_handler(client: Client, CallbackQuery):
    ...
    keybord = InlineKeyboardMarkup(
        [[InlineKeyboardButton("رجوع", callback_data="jzhfjgh5")]]
    )
    ...
    try:
        ...
        if isinstance(
            protection_list[command], list
        ):  # Multiple protections to be unlocked
...
async def mearhjc(client: Client, CallbackQuery):
    ...
    if protection_type in protection_lists:
        ...
        protection_type_name = protection_type.replace(
            "_", " "
        )  # Make the name user-friendly
        response_message = add_to_protection_list(
            protection_list, chat_id, protection_type_name
        )
        return await CallbackQuery.edit_message_text(
            response_message, reply_markup=keybord
        )
...
async def media_handler(client, message):
    ...
    media_type_map = {
        ...
        "sticker": {
            "muted": sticker_mut,
            "banned": sticker_ban,
            "locked": sticker_locked,
        },
        "forward": {
            "muted": forward_mut,
            "banned": forward_ban,
            "locked": forward_locked,
        },
    }
    ...
    for media_type, action_dict in media_type_map.items():
        if isinstance(message, media_type):  # Check if message is of the current type
            ...
            if group_id in action_dict["muted"]:
                await handle_action(
                    client, message, action_dict["muted"], "mute", media_type
                )
            elif group_id in action_dict["banned"]:
                await handle_action(
                    client, message, action_dict["banned"], "ban", media_type
                )
            elif group_id in action_dict["locked"]:
                await handle_action(
                    client, message, action_dict["locked"], "lock", media_type
                )
...
async def set_chat_permissions(
    client, message, lock_type, can_pin_messages, can_send_messages
):
    group_id = message.chat.id
    ...
    if (
        ...
        await client.set_chat_permissions(
            message.chat.id,
            ChatPermissions(
                can_pin_messages=can_pin_messages, can_send_messages=can_send_messages
            ),
        )
        await message.reply_text(
            f"• تم {lock_type} التثبيت بواسطه ↤︎「 {message.from_user.mention}"
        )
    else:
        return await message.reply_text(
...
async def lock_chat(client, message):
    await set_chat_permissions(
        client, message, "قفل", can_pin_messages=False, can_send_messages=False
    )
...
async def unlock_chat(client, message):
    await set_chat_permissions(
        client, message, "فتح", can_pin_messages=True, can_send_messages=True
    )
...
async def lock_pin(client, message):
    await set_chat_permissions(
        client, message, "قفل", can_pin_messages=False, can_send_messages=True
    )
...
async def set_chat_permissions(
    client, message, can_invite_users, can_send_media_messages, can_send_messages
):
    group_id = message.chat.id
    if group_id in mangof:
        return await message.reply_text(
...
async def caesarprivileges(client, message):
    ...
    if cae.status == ChatMemberStatus.OWNER:
        ...
        privilege_text = "\n".join(
            [
                f"{key}: {'✅' if value else '❌'}"
                for key, value in privileges_list.items()
            ]
        )
...
async def lock_unlock_handler(
    client, message, action_type, protection_list, action_message, success_msg
):
    if await is_authorized(client, message):
        if action_type == "lock":
...
async def muttlock(client, message):
    await lock_unlock_handler(
        client,
        message,
        "lock",
        muttof,
        "تم تعطيل التقييد بنجاح ✅🔒",
        "تم معطل من قبل🔒",
    )
...
async def gaaopen(client, message):
    await lock_unlock_handler(
        ...
        "unlock",
        gaaof,
        "تم فتح الطرد و الحظر بنجاح ✅🔓",
        "الطرد و الحظر مفعل من قبل ✅",
    )
