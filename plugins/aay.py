    ...
    if step == 1:
        if message.contact:
            ...
            if res == 0:
                ...
                await message.reply(
                    "**✧¦ ئەم ژمارەیە سنووردار کراوە، تکایە چەند کاتژمێرێکی تر هەوڵبدەوە♥️•**"
                )
            else:
                await message.reply(
                    "**✧¦ هەڵەیەکی ڕوویدا تکایە لە چەند خولەکێکی تر دووبارە هەوڵ بدەرەوە♥️•**"
                )
        else:
        ...
        if message.forward_from:
            try:
                ...
                if res == 0:
                    ...
                    await message.reply(
                        "**✧¦ هەڵەیەکی ڕوویدا تکایە دووبارە هەوڵ بدەرەوە♥️•**"
                    )
            except IndexError:
                await message.reply(
                    "**✧¦ ناتوانرێت کۆدەکە بدۆزرێتەوە، تکایە دڵنیابە لە فۆرواردی نامەکە.**"
                )
        else:
        ...
        if "بەڵێ" in text:
            ...
            steps[user_id] = 1
            await message.reply(
                "**✧¦ سڕینەوە هەڵوەشایەوە ♥️•**", reply_markup=ReplyKeyboardRemove()
            )
