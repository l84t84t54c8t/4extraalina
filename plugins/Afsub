...
async def set_channel(client: Client, message: Message):
    ...
    if not message.chat.has_protected_content or not (
        await app.get_chat_member(message.chat.id, message.from_user.id)
    ).status in [ChatMember.ADMINISTRATOR, ChatMember.OWNER]:
    ...
    if not channel_username:
        await message.reply_text(
            "Please provide the channel username (e.g., @YourChannel)."
        )
        return
