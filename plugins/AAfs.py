...
forcesub_collection2 = fsubdb.status_db.status

@app.on_message(filters.command(["fsub2", "forcesub"]) & filters.group)
async def set_forcesub(client: Client, message: Message):
    ...
    if len(message.command) == 2 and message.command[1].lower() in ["off", "disable"]:
        forcesub_collection2.delete_one({"chat_id": chat_id})
    ...
    try:
        ...
        forcesub_collection2.update_one(
            {"chat_id": chat_id},
...
async def check_forcesub(client: Client, message: Message):
    ...
    forcesub_data = forcesub_collection2.find_one({"chat_id": chat_id})
